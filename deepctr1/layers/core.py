...
class PredictionLayer(Layer):
    ...
    def __init__(self, task='binary', use_bias=True,multi_n=1, **kwargs):
        if task not in ["binary", "regression","multi-class"]:
            raise ValueError("task must be binary or regression or multi-class")
        self.task = task
    ...
    def build(self, input_shape):
        ...
        if self.use_bias:
            self.global_bias = self.add_weight(
                shape=(self.multi_n,), initializer=Zeros(), name="global_bias")
    ...
    def call(self, inputs, **kwargs):
        ...
        if self.task=="multi-class":
            x=tf.nn.softmax(x)
            output = tf.reshape(x, (-1, self.multi_n))
