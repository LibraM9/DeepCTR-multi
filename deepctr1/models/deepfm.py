...
def DeepFM(feature_dim_dict, embedding_size=8,
           ...
           init_std=0.0001, seed=1024, dnn_dropout=0, dnn_activation='relu', dnn_use_bn=False, task='binary',multi_n=1):
    ...
    :param task: str, ``"binary"`` for  binary logloss or  ``"regression"`` for regression loss ,"multi-class" for categorical_crossentropy
    ...
    linear_logit = get_linear_logit(linear_emb_list, dense_input_dict, l2_reg_linear,multi_n=multi_n)

    fm_input = concat_fun(deep_emb_list, axis=1)
    deep_input = tf.keras.layers.Flatten()(fm_input)
    fm_out = FM(multi_n=multi_n)(fm_input)
    deep_out = DNN(dnn_hidden_units, dnn_activation, l2_reg_dnn, dnn_dropout,
                   dnn_use_bn, seed)(deep_input)
    deep_logit = tf.keras.layers.Dense(
        multi_n, use_bias=False, activation=None)(deep_out)
    ...
    output = PredictionLayer(task,multi_n=multi_n)(final_logit)
